(() => {
  ...
  const getNeedAnalysis = () => {
    ...
    const headline = document.querySelector('#result h1')?.textContent || '';
    return { answers, scores: D, needIndex, total, level, headline };
  };
  ...
  const getMethodAnalysis = () => {
    ...
    const answers = a.map((v,index) => ({ question: qs[index]?.[0], answer: qs[index]?.[2]?.[Number(v)] ?? null, value: Number(v), profile: qs[index]?.[3]?.[Number(v)] ?? null }));
    ...
    return { answers, scores, primary: primary ? { key: primary, name: profiles[primary]?.name } : null, secondary: secondary ? { key: secondary, name: profiles[secondary]?.name } : null, strength: primary ? Math.round(scores[primary] / qs.length * 100) : 0 };
  };
  ...
  const renderAI = (analysis) => {
    ...
    panel.innerHTML = `<span class="ai-badge">INTERPRETASI PERSONAL TUTORIN</span><h2>Yang paling terlihat dari jawaban Anda</h2><p>${escapeHtml(analysis.summary)}</p><h3>Hal yang paling terlihat</h3>${listHtml(analysis.key_observations)}<h3>Yang bisa dilakukan orang tua</h3>${listHtml(analysis.parent_guidance)}<h3>Arahan untuk tutor</h3>${listHtml(analysis.tutor_guidance)}<h3>Langkah berikutnya</h3>${listHtml(analysis.next_steps)}`;
